...
function AddressCell({ address }: { address: string }) {
    ...
    return (
        ...
        >
            <span>{short}</span>
            <Copy className={`w-3 h-3 shrink-0 transition ${copied ? 'text-emerald-400' : 'text-zinc-600 group-hover:text-zinc-400'}`} />
        </button>
    );
}
...
export default function WithdrawalHistory({ data, advanceFilterOptions = [] }: any) {
    const { t } = useTranslations();

    const withdrawals: any[] = data?.data?.data ?? [];
    ...
    const total = meta?.total ?? withdrawals.length;
    const pending = withdrawals.filter(w => w.status === 'pending').length;
    const approved = withdrawals.filter(w => w.status === 'approved').length;
    const activeFilterCount = Object.values(filters).filter(Boolean).length;

    return (
        <UserLayout title={t('Withdrawal History')}>
            <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900">{t('Withdrawal History')}</h1>
                                            ...
                                            <div className="flex-1 min-w-0 space-y-1">
                                                ...
                                                <div className="flex items-center justify-between gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <AddressCell address={w.withdrawal_address ?? ''} />
                                                    </div>
                                                ...
                                                {/* Fee row */}
                                                {(w.fee_charged || w.amount) && (
                                                    <div className="flex items-center justify-between text-xs text-gray-500">
                                                        {w.fee_charged && w.fee_charged !== '0' && (
                                                            <span>
                                                                {t('Fee')}: {w.fee_charged}
                                                                {w.fee_pct ? ` (${w.fee_pct}%)` : ''}
                                                            </span>
