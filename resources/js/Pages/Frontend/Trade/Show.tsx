    ...
    return (
        <div className="relative flex items-center justify-center" style={{ width: 80, height: 80 }}>
            ...
            <div className="absolute flex items-center justify-center">
                ...
                <span className="text-[13px] text-slate-400 leading-none ml-0.5">s</span>
            </div>
        </div>
    )
}
...
function OrderPanel({
    ...
    return (
        <div className="space-y-5">
            <div>
                <p className="text-[13px] font-medium text-slate-600 mb-2">{t('Transaction Mode')}</p>
            ...
            <div>
                <p className="text-[13px] font-medium text-slate-600 mb-2">{t('Opening quantity')}</p>
            ...
            <div>
                <p className="text-[13px] font-medium text-slate-600 mb-2">{t('Opening time')}</p>
...
export default function TradeShow({
    ...
    // ── Trade detail modal for positions ──
        ...
        countdownRef.current = setInterval(() => {
            setActiveTrade((prev: any) => {
                ...
                if (next <= 0) {
                    ...
                    // Show settling phase (activeTrade=null, result=null → 'settling')
    ...
    // ── Shared positions list renderer ──
    const renderPositionsList = (orders: any[]) => (
        <>
            {/* Header */}
            ...
            {(orders ?? []).length === 0 ? (
                ...
                (orders ?? []).map((trade: any, idx: number) => (
                    <div
                        key={idx}
        ...
        <UserLayout title={`${crypto.symbol ?? '...'}/${defaultCurrency}`} back={route('market')}>
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 bg-white border-b border-slate-100 shrink-0" style={{ height: 48 }}>
                    ...
                    {/* ── Positions / K-Line toggle button ── */}
                    <button
                    ...
                    >
                        {showPositions ? (
                            <>
                                ...
                                <span className="text-xs font-semibold text-blue-500">{t('K-Line')}</span>
                            </>
                        ) : (
                            <>
                                <ClipboardList className="w-3.5 h-3.5 text-blue-500" />
                                <span className="text-xs font-semibold text-blue-500">{t('Positions')}</span>
                            </>
                        )}
                    </button>
                </div>
                ...
                {/* ── Tabs (hidden when positions view is active) ── */}
                {!showPositions && (
                ...
                <div className="flex-1 overflow-hidden flex flex-col min-h-0" style={{ paddingBottom: 58 }}>
                    ...
                    {showPositions && (
                        <div className="flex-1 overflow-y-auto bg-white">
                            {renderPositionsList(ordersProp ?? [])}
                        </div>
                    )}
                            ...
                            {activeTab === 'trade' && (
                                <div className="flex-1 min-h-0 flex flex-col">
                                    <div className="flex-1 min-h-0">
                                        <TradingViewWidget symbol={binanceSymbol} />
                                    </div>
                                </div>
                            )}
                                        ...
                                        <div key={c.id ?? c.symbol}>
                                            <Link href={route('user.trade.show', { crypto: c.symbol })}
                                                ...
                                                <div className="flex items-center gap-3">
                                                    {c.images?.small
                                                        ? <img src={c.images.small} alt={c.symbol} className="w-9 h-9 rounded-full" />
                                                        : <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500">{c.symbol?.[0]}</div>
                                                    }
                                                    <div>
                ...
                <div className="fixed left-0 right-0 z-[90] bg-white/95 backdrop-blur border-t border-slate-200" style={{ bottom: NAV_H }}>
                    <div style={{
                        ...
                        gap: isMobile ? 8 : 'clamp(8px, 6vw, 64px)',
                        padding: isMobile ? '6px 10px' : '6px clamp(8px, 6vw, 64px)',
                        width: '100%',
                        ...
                        <button onClick={() => handleTrade('long')} disabled={!!activeTrade}
                            style={{ flex: isMobile ? 1 : 'none', width: isMobile ? undefined : 'clamp(80px, 22vw, 200px)', height: 44, borderRadius: 8, fontSize: isMobile ? 13 : 'clamp(11px, 3vw, 15px)' as any, fontWeight: 700, border: 'none', cursor: activeTrade ? 'not-allowed' : 'pointer', background: activeTrade ? '#e2e8f0' : '#22c55e', color: activeTrade ? '#94a3b8' : '#fff', flexShrink: 0, whiteSpace: 'nowrap' }}>
                            {t('Buy Long')}
                        </button>
                        <button onClick={() => handleTrade('short')} disabled={!!activeTrade}
                            style={{ flex: isMobile ? 1 : 'none', width: isMobile ? undefined : 'clamp(80px, 22vw, 200px)', height: 44, borderRadius: 8, fontSize: isMobile ? 13 : 'clamp(11px, 3vw, 15px)' as any, fontWeight: 700, border: 'none', cursor: activeTrade ? 'not-allowed' : 'pointer', background: activeTrade ? '#e2e8f0' : '#f87171', color: activeTrade ? '#94a3b8' : '#fff', flexShrink: 0, whiteSpace: 'nowrap' }}>
                            {t('Sell Short')}
                        </button>
                    </div>
                </div>
                    ...
                    {cryptos.map((c: any, i: number) => (
                        <div key={c.id ?? c.symbol}>
                            <Link href={route('user.trade.show', { crypto: c.symbol })} onClick={() => setShowCoinDrawer(false)}
                                ...
                                <div className="flex items-center gap-2.5">
                                    {c.images?.small
                                        ? <img src={c.images.small} alt={c.symbol} className="w-8 h-8 rounded-full" />
                                        : <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600">{c.symbol?.[0]}</div>
                                    }
                                    <div>
            ...
            {/* ── Trade Modal (new style: images 1 & 2) ── */}
            {showTradeModal && (
                <TradeModal
